var e={meta:{id:`s11`,number:11,title:`深度学习 & CNN`,icon:`Layers`,courseware:`14 计算机视觉.pptx`,examRefs:`概念题为主`},calloutText:`这章解决什么问题？计算机怎么"看"懂一张照片？——你要告诉它猫长什么样，不能靠规则（"猫有尖耳朵、胡须、四条腿"太难写），而是让它自己从几万张猫图里学。CNN（卷积神经网络）就是专门为图像设计的"眼睛"。`,sections:[{id:`s11-s2`,title:`11.1 先理解直觉`,content:[{type:`prose`,html:`<p>你看一张猫的照片，不会从像素 0 开始逐个检查——你的大脑会自动识别"这里有毛的纹理""那里有耳朵的形状""那边是眼睛"。CNN 做的就是类似的事：</p>`},{type:`prose`,html:`<ul>
...
</ul>`}]},{id:`s11-s3`,title:`11.2 核心概念`,content:[{type:`prose`,html:`<p><strong>感知器 —— 神经网络的最小单元</strong></p>`},{type:`prose`,html:`<p>一个感知器就像一个投票机：每个输入乘以权重，加总后过一道"门槛"（激活函数），输出最终决定。</p>`},{type:`mermaid`,id:`m-3h4ny2`,chart:`flowchart LR
    X1["x1"] -->|"x w1"| SUM["加权求和"]
    X2["x2"] -->|"x w2"| SUM
    X3["x3"] -->|"x w3"| SUM
    B["偏置 b"] --> SUM
    SUM -->|"加权和"| F["激活函数 f"]
    F -->|"输出"| Y["y"]`},{type:`prose`,html:`<p style="text-align:center">$$y = f(w_1 x_1 + w_2 x_2 + w_3 x_3 + b)$$</p>`},{type:`prose`,html:`<p><strong>具体例子：判断今天要不要出门</strong></p>`},{type:`code`,language:``,code:`输入 x₁ = 1（今天下雨），权重 w₁ = —3（下雨很减分）
输入 x₂ = 1（有重要约会），权重 w₂ = +5（约会很加分）
偏置 b = —1

加权和 = (—3)×1 + 5×1 + (—1) = 1
激活函数（阶跃）：1 > 0 → 输出 1 → "出门！"
                                  → 输出 0 → "不出门"`},{type:`prose`,html:`<p><strong>为什么需要激活函数（非线性）？</strong></p>`},{type:`prose`,html:`<p>直觉：XOR 问题。如果只用线性（没有激活函数），你画一条直线永远分不开 XOR 的两类点。加了非线性激活函数（如 ReLU、Sigmoid），网络就能"弯折"决策边界，像用橡皮泥捏出复杂形状。</p>`},{type:`callout`,variant:`tip`,text:`一句话记住：没有激活函数，100 层线性层等价于 1 层——没有非线性，深度网络就废了。`},{type:`prose`,html:`<p><strong>CNN 的核心操作</strong></p>`},{type:`table`,headers:[],rows:[[`操作`,`作用`,`生活类比`],[`卷积`,`用小窗口（卷积核）滑过图片，提取局部特征`,`拿放大镜检查图片每一小块有没有边缘`],[`池化`,`缩小特征图，降维 + 保留主要信息`,`把 4K 照片压成 1080p——主要内容还在`],[`全连接`,`把所有特征组合起来做分类`,`看了耳朵、眼睛、胡须 → 综合判断是猫`],[`Softmax`,`把输出变成概率（所有概率加起来 = 1）`,`"70% 是猫，20% 是狗，10% 是兔子"`]]}]},{id:`s11-s4`,title:`11.3 动手试试 —— 卷积计算逐步推演`,content:[{type:`prose`,html:`<p><strong>这是 2023 期末的考点！必须会逐元素算卷积。</strong></p>`},{type:`prose`,html:`<p><strong>给定：4×4 输入矩阵，2×2 卷积核，步长 = 1</strong></p>`},{type:`prose`,html:`<div class="grid-2">
<div>
<table>
...
</div>`},{type:`prose`,html:`<p><strong>卷积过程：卷积核在输入上滑动，每次做"对应位置相乘再求和"。</strong></p>`},{type:`details`,summary:`位置 (0,0)：卷积核覆盖左上角 2×2`,body:`<d\betails>
...
</d\betails>`},{type:`details`,summary:`位置 (0,1)：卷积核右移一步`,body:`<d\betails>
...
</d\betails>`},{type:`details`,summary:`位置 (0,2)：再右移一步`,body:`<d\betails>
...
</d\betails>`},{type:`details`,summary:`位置 (1,0)：卷积核下移一步，回到最左`,body:`<d\betails>
...
</d\betails>`},{type:`prose`,html:`<p><strong>继续算完所有 9 个位置（3×3 输出矩阵）：</strong></p>`},{type:`code`,language:``,code:`位置 (1,1)：覆盖 {1,2 / 0,1} → 1×1 + 2×0 + 0×0 + 1×(—1) = 1—1 = 0
位置 (1,2)：覆盖 {2,3 / 1,2} → 2×1 + 3×0 + 1×0 + 2×(—1) = 2—2 = 0
位置 (2,0)：覆盖 {3,0 / 2,3} → 3×1 + 0×0 + 2×0 + 3×(—1) = 3—3 = 0
位置 (2,1)：覆盖 {0,1 / 3,0} → 0×1 + 1×0 + 3×0 + 0×(—1) = 0
位置 (2,2)：覆盖 {1,2 / 0,1} → 1×1 + 2×0 + 0×0 + 1×(—1) = 1—1 = 0`},{type:`table`,headers:[],rows:[[`卷积输出 3×3`],[`0`,`0`,`0`],[`0`,`0`,`0`],[`0`,`0`,`0`]]},{type:`prose`,html:`<p style="font-size:var(--text-xs);color:var(--color-text-secondary)">这个特定核是边缘检测器——平坦区域输出 0</p>`},{type:`callout`,variant:`tip`,text:`考试技巧：实际考试会给更简单的数字，不用慌。关键是记住：滑动窗口 → 对应位置相乘 → 求和 → 移一位再来。步长 1 时输出尺寸 = (输入尺寸 — 卷积核尺寸) + 1。`}]},{id:`s11-s5`,title:`11.4 动手试试 —— 最大池化（Max Pooling）`,content:[{type:`prose`,html:`<p><strong>2023 期末真题矩阵：池化窗口 2×2，步长 2</strong></p>`},{type:`prose`,html:`<div class="grid-2">
<div>
<table>
...
</div>`},{type:`prose`,html:`<p><strong>池化输出尺寸公式：</strong></p>`},{type:`code`,language:``,code:`输出 = (输入 — 池化窗口) / 步长 + 1
     = (4 — 2) / 2 + 1
     = 2/2 + 1
     = 1 + 1
     = 2  → 4×4 变成 2×2，正好！`}]},{id:`s11-s7`,title:`11.5 复习策略`,content:[{type:`prose`,html:`<h2 id="s-priority">十四、最终复习优先级</h2>`},{type:`card`,title:`大题 25-40分 第一优先级：必须会做大题`,body:`<div class="card tier-card tier-1">
...
</div>`},{type:`card`,title:`简答 15-25分 第二优先级：必须会标准简答`,body:`<div class="card tier-card tier-2">
...
</div>`},{type:`card`,title:`概念 10-15分 第三优先级：范围内保底`,body:`<div class="card tier-card tier-3">

<ol>
<li>CSP 建模和回溯优化方法</li>
<li>Minimax 和 $\x07alpha$-$\b\beta$ 剪枝</li>
<li>命题逻辑模型检查和归结</li>
<li>HMM 基本结构</li>
<li>感知器算法</li>
<li>线性回归、线性分类</li>
...
</div>`},{type:`prose`,html:`<h2 id="s-rounds">十五、四轮复习顺序</h2>`},{type:`card`,title:`🔵 第 1 轮：建立主线`,body:`<div class="card" style="border-left:4px solid var(--color-accent)">
...
</div>`},{type:`card`,title:`🔴 第 2 轮：攻计算题`,body:`<div class="card" style="border-left:4px solid var(--color-danger)">
...
</div>`},{type:`card`,title:`🟡 第 3 轮：攻证明题`,body:`<div class="card" style="border-left:4px solid var(--color-warning)">
...
</div>`},{type:`card`,title:`🟢 第 4 轮：背简答题模板`,body:`<div class="card" style="border-left:4px solid var(--color-success)">
...
</div>`},{type:`prose`,html:`<h2 id="s-checklist">十六、考前自检清单</h2>`},{type:`prose`,html:`<p class="checklist-counter">已完成 <strong id="checkCount">0</strong> / 14 项</p>`},{type:`prose`,html:`<ul class="checklist" id="checklist">
...
<li data-id="5"><span class="check-box"></span><span class="check-label">给博弈树，填 <strong>Minimax</strong> 值并判断 <strong>$\x07alpha$-$\b\beta$ 剪枝</strong></span></li>
...
</ul>`},{type:`prose`,html:`<blockquote style="font-size:var(--text-lg);padding:var(--space-5);">
...
</blockquote>`},{type:`prose`,html:`<h2 id="s-index">附：历年真题 &amp; 课件索引</h2>`}]},{id:`s11-s8`,title:`11.6 考试怎么考`,content:[{type:`prose`,html:`<h4>考试题型与分值</h4>`},{type:`table`,headers:[],rows:[[`题型`,`分值`,`考察点`],[`卷积输出尺寸计算`,`3-5 分（计算）`,`给输入尺寸、卷积核尺寸、步长、padding，算输出尺寸`],[`卷积逐元素计算`,`5-8 分（大题）`,`给 4×4 或 3×3 矩阵 + 2×2 卷积核，步长=1，手动算每个输出值`],[`最大池化计算`,`3-5 分（计算）`,`给特征矩阵，池化窗口 2×2 步长 2，算池化输出`],[`CNN 组件作用`,`3-5 分（简答）`,`卷积/池化/全连接/Softmax 各自的作用`],[`激活函数作用`,`2-3 分（简答）`,`为什么需要非线性激活函数`]]},{type:`prose`,html:`<h4>得分点</h4>`},{type:`prose`,html:`<ul>
...
</ul>`},{type:`prose`,html:`<h4>常见错误</h4>`},{type:`callout`,variant:`danger`,text:`卷积输出尺寸忘记 +1：公式最后有 +1，很多人都漏掉。例如 4-2+1=3（不是 2）。验证：4×4 用 2×2 核步长 1，卷积核从位置 0 滑到位置 2，共 3 个位置 → 输出 3×3。`},{type:`callout`,variant:`danger`,text:`卷积和池化混用：卷积是"相乘再求和"（加权组合），池化是"取最大值"（选择操作）。前者产生新特征，后者只是压缩。`},{type:`callout`,variant:`danger`,text:`忘记标注步长：输出尺寸取决于步长。同样 4×4 输入 + 2×2 核，步长 1→3×3，步长 2→2×2。考试先看清步长再算。`},{type:`prose`,html:`<h4>答题模板</h4>`},{type:`card`,title:`卷积计算 答题格式`,body:`<div class="card">
...
</div>`}]},{id:`s11-s9`,title:`11.7 真题演练`,content:[{type:`examQuestions`,ids:[`eq-s11-29`,`eq-s11-30`]}]},{id:`s11-s10`,title:`11.8 小测验`,content:[{type:`quiz`,ids:[`q-s11-25`,`q-s11-26`]}]}]};export{e as default};