var e={meta:{id:`s10`,number:10,title:`虚拟内存 VM`,icon:`HardDrive`,courseware:`10 虚拟内存.pptx`,examRefs:`联合大题 ~25分 · 第一优先级`},calloutText:`这章解决什么问题？每个程序都以为自己独占了整个内存，但实际上一台电脑上跑着几十个程序，物理内存只有 8GB。怎么让每个程序都"以为"自己有 4GB 可用？答案是虚拟内存——它给每个程序编了一个假地址，然后悄悄地翻译成真地址。这一章就是学这个"翻译系统"怎么工作。`,sections:[{id:`s10-s1`,title:`10.0 本章在考试中的位置`,content:[{type:`prose`,html:`<p style="color:var(--color-text-secondary);font-size:var(--text-sm)">课件：第25-26讲 · 期末最大考点 · 与 Cache 联合出 25 分大题（TLB+页表+Cache 联合地址翻译）· 套路固定，掌握了流程就能拿大部分分</p>`},{type:`callout`,variant:`tip`,text:`核心一句话：虚拟地址 VA → TLB 查 VPN→PPN（找不到就查页表，再找不到就缺页处理）→ 拼出物理地址 PA → PA 再拆给 Cache 查数据。学完这章，把流程图背下来就成功了一半。`}]},{id:`s10-s2`,title:`10.1 先理解直觉——为什么需要虚拟内存？`,content:[{type:`prose`,html:`<p>想象你是一个学生，去图书馆自习。你有 3 本书要同时看，但书桌面积只够平铺 1 本书。怎么办？你把当下正看的那页摊在桌上，其他书摞在一边，不看的就还回书架（书架=硬盘）。虽然书桌面积小，但你觉得"想看什么书都能随时拿到"——这就是虚拟内存的核心体验。</p>`},{type:`prose`,html:`<p><strong>虚拟内存的三大功能：</strong></p>`},{type:`prose`,html:`<ol><li><strong>高效使用主存</strong>——DRAM 不够用，就把磁盘当"后备仓库"。只有程序真正用到的页才调入内存（demand paging）。</li><li><strong>简化内存管理</strong>——每个进程都有统一的地址空间布局（代码段在低地址、堆往上涨、栈往下跌、共享库在中间）。链接器只需要处理虚拟地址，不需要知道物理地址。</li><li><strong>内存保护</strong>——进程 A 不能读写进程 B 的内存。PTE 里有许可位（READ/WRITE/SUP），违例就 SIGSEGV（段错误）。</li></ol>`},{type:`prose`,html:`<p><strong>物理地址 vs 虚拟地址：</strong></p>`},{type:`table`,headers:[],rows:[[``,`虚拟地址 VA`,`物理地址 PA`],[`谁看到的？`,`程序（CPU 发出的）`,`内存芯片（DRAM 看到的）`],[`谁翻译？`,`MMU（内存管理单元，CPU 内部硬件）`,`不需要翻译`],[`地址空间大小`,`由 CPU 位数决定（如 32 位 → 4GB）`,`由实际插了多少内存决定`],[`每个进程是否独立？`,`是，每个进程有独立地址空间`,`所有进程共享`]]}]},{id:`s10-s3`,title:`10.2 核心概念——页、页表、PTE`,content:[{type:`card`,title:`VM 核心术语——第一组：地址与页`,body:`<div class="card">
<table>
...
</div>`},{type:`card`,title:`VM 核心术语——第二组：地址切分`,body:`<div class="card">
<table>
<tr><th>术语</th><th>符号</th><th>含义</th></tr>
...
</div>`},{type:`card`,title:`VM 核心术语——第三组：翻译设施`,body:`<div class="card">
<table>
<tr><th>术语</th><th>符号</th><th>含义</th></tr>
...
</div>`},{type:`card`,title:`地址格式——VA 和 PA 怎么切？`,body:`<div class="card">
...
<p><strong>例子：</strong>VA=20位, PA=16位, 页大小=256字节=2^8<br/>→ p=8, VPN=12位, VPO=8位, PPN=8位, PPO=8位</p>
</div>`},{type:`prose`,html:`<p><strong>PTE 的结构：</strong>一个 PTE 通常 4 或 8 字节，包含：</p><ul><li><strong>Valid bit（1位）：</strong>=1 表示该页在物理内存中，=0 表示页在磁盘上或未分配</li><li><strong>PPN（若干位）：</strong>如果 valid=1，这是物理页号；如果 valid=0，这个字段可能存放磁盘地址</li><li><strong>权限位：</strong>READ / WRITE / SUP（是否内核模式才能访问）</li></ul>`},{type:`callout`,variant:`warning`,text:`常见错误：把 VA 的位宽和 PA 的位宽搞混。VA 有 n 位（如 20位），PA 有 m 位（如 16位）。拆分时：VA 的 VPO 位数 = PA 的 PPO 位数 = log2(页大小)。VPO=PPO 这一点是永恒的。`}]},{id:`s10-s4`,title:`10.3 地址翻译全流程——考试必画图`,content:[{type:`prose`,html:`<p><strong>这是整个 Computer System 课程最重要的一张图，期末 25 分大题就是走这个过程。必须能默画。</strong></p>`},{type:`mermaid`,id:`s10-addr-trans`,chart:`flowchart TD
    ...
    MEM --> RESULT`},{type:`prose`,html:`<p><strong>流程文字版（考试写步骤用）：</strong></p><ol><li>CPU 发出虚拟地址 VA = {VPN, VPO}</li><li>用 VPO 的若干低位作为 TLBI，查 TLB 对应组</li><li><strong>TLB Hit：</strong>直接得到 PPN</li><li><strong>TLB Miss：</strong>去内存查页表 PTE[VPN]，得到 PPN（如果 PTE.Valid=0 则发生 Page Fault，从磁盘调入页后再重试）</li><li>拼出物理地址 PA = {PPN, PPO}，其中 PPO = VPO</li><li>以 PA 访问 L1 Cache（把 PA 拆成 CT | CI | CO）</li><li>Cache Hit → 返回数据；Cache Miss → 访问主存 → 装入 Cache → 返回数据</li></ol>`},{type:`callout`,variant:`tip`,text:`死记硬背也要记住：VPO = PPO。翻译只改变页号，页内偏移不变。这让你在计算 PA 时可以省一步。`},{type:`callout`,variant:`warning`,text:`考试踩坑点：VA 的 VPO 低几位可能用作 TLBI（因为 TLB 也是按组索引的），但剩下的才是真正的"页内偏移"。比如 20位VA, 256B页→VPO=8位, TLB有4组→TLBI=2位（来自VPO的低2位）。这些位既用于 TLB 索引，也是 VPO 的一部分——不要搞混。`}]},{id:`s10-s5`,title:`10.4 TLB——地址翻译的"Cache"`,content:[{type:`prose`,html:`<p><strong>TLB（Translation Lookaside Buffer）就是专门缓存 PTE 的小型 Cache。</strong>因为每次访存都要先翻译地址，如果不缓存 PTE，每次访存都要额外访问一次页表（在内存里）——太慢了。</p>`},{type:`card`,title:`TLB 的结构`,body:`<div class="card">
...
</div>`}]},{id:`s10-s6`,title:`10.5 多级页表——解决页表过大的问题`,content:[{type:`prose`,html:`<p><strong>问题：</strong>假设 48 位虚拟地址空间，4KB 页，每 PTE 8 字节。需要多少个 PTE？</p><p>$\\text{总页数} = 2^{48} / 2^{12} = 2^{36}$ 个页。$\\text{单级页表大小} = 2^{36} \\times 8\\text{B} = 512\\text{GB}$！这比整个物理内存还大，根本存不下。</p>`},{type:`card`,title:`多级页表方案——只存"在用"的部分`,body:`<div class="card">
...
</div>`},{type:`callout`,variant:`tip`,text:`考试中多级页表一般考概念或简单计算。不会让你真实地走一遍四级页表翻译——那太复杂了。重点是理解"为什么需要多级"（单级太大）和"多级怎么节省内存"（只分配活跃子表）。`}]},{id:`s10-s7`,title:`10.6 缺页异常（Page Fault）& 写时拷贝（COW）`,content:[{type:`card`,title:`Page Fault 处理六步`,body:`<div class="card">
<ol>
...
</div>`},{type:`card`,title:`Copy-on-Write（写时拷贝）——fork() 的优化`,body:`<div class="card">
...
</div>`},{type:`table`,headers:[],rows:[[`故障类型`,`原因`,`能否恢复？`,`结果`],[`正常缺页`,`PTE.Valid=0，页在磁盘`,`能`,`调入页后重试成功`],[`地址越界`,`VA 超过进程地址空间`,`不能`,`SIGSEGV，进程终止`],[`访问越级`,`用户态访问内核页`,`不能`,`SIGSEGV，进程终止`],[`访问越权`,`写只读段（如 .text）`,`不能`,`SIGSEGV，进程终止`]]}]},{id:`s10-s8`,title:`10.7 动手试试——联合翻译大题完整演练`,content:[{type:`prose`,html:`<p><strong>这是 2025 期末真题题型（25 分），2024 同款（25 分）。</strong>下面我们完整地走一遍考试会出现的题型。</p>`},{type:`card`,title:`真题题型：第一步——算基本参数`,body:`<div class="card">
...
</div>`},{type:`card`,title:`真题题型：第二步——给定 VA=0x1E9A7，走全程`,body:`<div class="card">
...
</div>`},{type:`card`,title:`真题题型：第三步——第二个 VA=0x1C7C3`,body:`<div class="card">
...
</div>`},{type:`prose`,html:`<p><strong>练习步骤总结——考试时按这个顺序写：</strong></p><ol><li>先算所有字段的位宽（VPO/VPN/PPO/PPN/TLBI/TLBT/CO/CI/CT）→ 写在草稿上</li><li>VA → 二进制 → 切开 VPN/VPO</li><li>从 VPN 提取 TLBI/TLBT → 查 TLB</li><li>TLB Miss 时查页表（注意缺页判断）</li><li>拼 PA = PPN | VPO</li><li>PA → 二进制 → 切开 CT/CI/CO → 查 Cache</li><li>Cache Hit → 返回数据；Miss → 填 N/A</li></ol>`}]},{id:`s10-s9`,title:`10.8 考试怎么考`,content:[{type:`prose`,html:`<h4>题型与分值</h4>`},{type:`table`,headers:[],rows:[[`题型`,`分值`,`考查内容`],[`基本参数计算`,`4~5 分`,`给 VA/PA 位宽和页大小，求各字段位宽`],[`联合翻译大题（第一部分）`,`~10 分`,`给 VA, TLB, 页表 → 判 TLB hit/miss, 缺页, 求 PPN/PA`],[`联合翻译大题（第二部分）`,`~10 分`,`PA 拆给 Cache → 判 Cache hit/miss, 返回字节值`]]},{type:`prose`,html:`<h4>得分点</h4>`},{type:`prose`,html:`<ul><li><strong>位宽计算全对 = 送分项：</strong>VPO=log2(页大小) 这个永不变，其他位宽从总宽度减出来。</li><li><strong>六进制转换要熟练：</strong>VA 转二进制、切段、查表、再拼回十六进制——每一步都可能在中间扣分。建议每一步都写二进制，减少出错。</li><li><strong>表格查对要仔细：</strong>TLB/页表/Cache 表都有 Valid bit，先检查 Valid=1 再看 Tag 匹不匹配。很多人忘了 Valid bit。</li></ul>`},{type:`prose`,html:`<h4>常见错误</h4>`},{type:`callout`,variant:`danger`,text:`致命错误：忘记检查 Valid bit。PTE 的 valid=0 不代表"没映射"就跳过——它是缺页！要填"是"缺页，且 PPN 填 N/A。Cache 的 valid=0 代表该行是空的——视为 Miss。`},{type:`callout`,variant:`warning`,text:`位宽错误：TLBI 和 TLBT 是从 VPN 里分的（不是从完整的 VA），Cache 的 CT/CI/CO 是从 PA 里分的（不是从 VA）。两套拆分独立，不要混。`}]},{id:`s10-s10`,title:`10.10 真题演练`,content:[{type:`examQuestions`,ids:[`eq-s10-1`,`eq-s10-2`]}]},{id:`s10-s11`,title:`10.11 小测验`,content:[{type:`quiz`,ids:[`q-s10-1`,`q-s10-2`]}]},{id:`s10-s-trace`,title:`地址翻译全链路实战`,subtitle:`从一个虚拟地址的旅程看透整个存储体系`,content:[{type:`prose`,html:`<p>## 从一个地址的旅程开始<br><br>你在淘宝下单，收货地址填的是「湖南省长沙市岳麓区麓山南路 208 号」（**虚拟地址 VA**）。快递公司不会拿着这个地址直接去找仓库——它先查自己的**配送站速查表（TLB）**：「麓山南路这片最近送过没有？」如果查到了，直接就知道对应的仓库编号。<br><br>如果 TLB 里没有，就得去翻**全国地址库（页表）**——这是个巨大的表，查起来慢，但什么地址都能查到。找到对应的实际仓库位置（**物理地址 PA**）后，顺手记到配送站速查表里（**TLB 更新**），下次就快了。<br><br>万一全国地址库里也查不到——说明这个地址对应的仓库压根没货（**缺页异常**），那就得从总仓（**磁盘**）调货了。<br><br>找到仓库（物理地址）后，快递员去仓库取货——但他不是翻遍整个仓库，而是直奔特定货架（**Cache 组**），看标签是不是自己要的货（**Cache 标记比对**），拿到了就送出去（**数据返回**）。<br><br>**这个比喻对应关系**：<br>- 收货地址 = 虚拟地址 VA<br>- 配送站速查表 = TLB（快，容量小）<br>- 全国地址库 = 页表（慢，容量大）<br>- 仓库编号 = 物理地址 PA<br>- 货架 = Cache<br>- 总仓 = 磁盘<br><br>这就是地址翻译的**全链路**。下面我们把它拆开，一步一步走。先看全景，再看零件——这是理解虚拟内存最有效的方式。</p>`},{type:`card`,body:`<div class="card">
<p>\`\`\`</p>
...
<p>- **这是考试最爱挖坑的地方！**</p>
</div>`},{type:`code`,language:`text`,code:`给定配置（仿 2025 期末真题，参数简化）：
  虚拟地址 VA = 20 bit
  物理地址 PA = 16 bit
  页面大小 = 256 B
...
A: 停止翻译，触发缺页异常。不要继续拼 PA、不要查 Cache。
   PPN 此时无效，写 "N/A" 或 "缺页"。`},{type:`details`,summary:`练习 1：参数位区间计算（仿 2025 真题结构）`,body:`<details>
...
- 翻译的本质：把 VPN 替换为 PPN，低位原封不动拼接上去`}]}]};export{e as default};