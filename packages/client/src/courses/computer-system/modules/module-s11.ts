...
const content: ModuleContent = {
  meta,
  calloutText: "这章解决什么问题？代码写对了，但跑得太慢。怎么不换硬件就让程序快几倍？答案是对 Cache 友好、减少循环开销、利用 CPU 的指令级并行能力。这一章是 Cache 知识的直接应用。",
  sections: [
  {
    id: "s11-s1",
    title: "11.0 本章在考试中的位置",
    content: [
      {
        type: "prose",
        html: "<p style=\"color:var(--color-text-secondary);font-size:var(--text-sm)\">课件：第27-28讲 · 分析题小问 ~5分 · 和 Cache 局部性紧密相关 · 常和链接/信号一起出在分析题中</p>"
      },
      {
        type: "callout",
        variant: "tip",
        text: "这一章不算重头戏，但\"给一段代码让你优化\"是分析题里的经典小问。记住三个大招：循环展开、减少函数调用、分块/缓存优化。看完这章你就能秒杀这5分。"
      }
    ]
  },
  {
    id: "s11-s2",
    title: "11.1 先理解直觉——为什么同样的结果，有的代码快10倍？",
    content: [
      {
        type: "prose",
        html: "<p>想象你在抄写一篇文章。一种抄法是看一个字写一个字；另一种是看一句话默记下来再写一整句。显然后者更快——因为你减少了\"抬头看原文\"的次数。</p>"
      },
      {
        type: "prose",
        html: "<p>计算机也一样。性能优化就是减少\"不必要的开销\"：</p><ul><li><strong>循环开销：</strong>每次循环都要比较 i < n、跳转。如果循环体本身很简单（比如只做一次加法），这些开销占比很大。</li><li><strong>Cache Miss：</strong>数据不在 Cache 里，要等几百个周期从内存取。</li><li><strong>函数调用开销：</strong>call/ret 需要压栈、跳转、建立栈帧——如果函数体就一行，纯浪费。</li></ul>"
      },
      {
        type: "prose",
        html: "<p><strong>优化的三个方向：</strong>(1) 减少循环开销 (2) 提高 Cache 命中率 (3) 利用指令级并行。下面一个一个讲。</p>"
      }
    ]
  },
  {
    id: "s11-s3",
    title: "11.2 循环展开（Loop Unrolling）",
    content: [
      {
        type: "card",
        title: "核心思想",
...
for (; i < n; i++)
    sum += a[i];`
      ...
      {
        type: "prose",
        html: "<p><strong>能快多少？</strong>对简单运算（如求和），展开 4 次能减少约 75% 的循环控制开销。但如果循环体本身就很重（如做复杂运算），循环开销占比小，展开效果就不明显。</p>"
      },
      {
        type: "mermaid",
        id: "s11-loop-unroll-compare",
        chart: `graph LR
  ...
  {
    id: "s11-s4",
    title: "11.3 分块/平铺（Blocking/Tiling）——让 Cache 高兴",
    content: [
      {
        type: "prose",
        html: "<p>这是<strong>矩阵运算的终极优化</strong>，也是 Cache 知识最直接的应用。</p>"
      },
      {
        type: "card",
        title: "分块的核心思想",
...
for (int ii = 0; ii < N; ii += B)
    for (int jj = 0; jj < N; jj += B)
      ...
      },
      {
        type: "callout",
        variant: "warning",
        text: "分块的前提是 3B^2 < C（三个矩阵的子块都能放进 Cache）。如果 B 太大，子块本身都装不下，分块反而更慢。实际中 B 通常取 32~64。"
      }
    ]
  },
  {
    id: "s11-s5",
    title: "11.4 减少函数调用 & 其他技巧",
    content: [
      {
        type: "card",
        title: "减少函数调用",
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: "prose",
        html: "<p>给你一段代码（通常是循环或数组操作），问\"如何优化，为什么能提高性能？\"（如 2025 期末分析题第5小问）。</p>"
      },
      {
        type: "prose",
        html: "<h4>答题模板</h4>"
...
4. （如有）提及约束条件：[如分块要求 3B^2 < C]`
      },
      {
        type: "callout",
        variant: "tip",
        text: "这题是送分题——只要你能把优化方案关联到\"Cache局部性\"或\"减少循环开销\"就有分。不需要写完整代码，用文字描述清楚即可。"
      }
    ]
  },
  {
    ...
    title: "11.7 真题演练",
    content: [
      { type: "examQuestions", ids: ["eq-s11-1", "eq-s11-2"] }
    ]
  },
  {
    id: "s11-s8",
