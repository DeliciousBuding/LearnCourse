...
const content: ModuleContent = {
  meta,
  calloutText: "这章解决什么问题？让计算机从一堆例子中自己\"总结规律\"，而不是我们一条一条告诉它规则。就像你看了 100 封垃圾邮件，自然就知道垃圾邮件长什么样——机器学习做的就是这件事。",
  sections: [
  {
    "id": "s9-s2",
    "title": "9.1 先理解直觉",
    "content": [
      {
        "type": "prose",
        "html": "<p>假如你去水果店，老板给了你一堆水果，让你分类哪些是苹果、哪些是橘子。你可以看颜色（红/橙），摸硬度（硬/软），闻气味。每次做对或做错，老板会告诉你\"对了\"或\"不对\"，你就慢慢学会了。</p>"
      },
      {
        "type": "prose",
  ...
  },
  {
    ...
    "content": [
      {
        "type": "prose",
        "html": "<p><strong>ML 四种类型（2025 期末 9 分，真题直接考了）</strong></p>"
      },
      {
        "type": "table",
        "headers": [],
        "rows": [
          ...
          [
            "监督学习",
            "有标签 (x, y)",
            "给你 1000 封邮件，每封都标了\"垃圾\"或\"正常\"，训练分类器"
          ],
          [
            "无监督学习",
            "无标签 (只有 x)",
            "给你 10 万条用户购物记录，自动把用户分成\"学生党\"\"上班族\"\"宝妈\"几类"
          ],
          [
            "半监督学习",
            "少量标签 + 大量无标签",
            "标注了 100 张猫图，还有 10 万张未标注图，让模型自己利用未标注数据提高准确率"
          ],
          [
            "强化学习",
            "奖励信号",
            "AlphaGo 下围棋：赢了 +1，输了 -1，自己跟自己下几百万盘学会的"
          ]
        ]
      },
      {
        "type": "prose",
        "html": "<p><strong>信息熵 —— \"一堆东西有多乱？\"</strong></p>"
      },
      {
        "type": "prose",
        "html": "<p>想象你有一袋水果：全是苹果（纯的），熵 = 0；一半苹果一半橘子（最乱的），熵最大。</p>"
      },
      {
        "type": "code",
        "language": "",
        "code": "信息熵公式：H(D) = — Σ pk × log₂(pk)\n\n例子：10 个水果，7 个苹果 3 个橘子\nH = —(0.7 × log₂(0.7) + 0.3 × log₂(0.3))\n  = —(0.7 × (—0.5146) + 0.3 × (—1.7370))\n  = —(—0.3602 — 0.5211)\n  = 0.8813\n\n极端情况：\n  全苹果（10正0负）：H = —(1 × log₂1 + 0) = —(1×0 + 0) = 0 ✓ 纯的\n  各一半（5正5负）：H = —(0.5×(—1) + 0.5×(—1)) = 1.0 ✓ 最乱"
      },
      {
        "type": "prose",
        "html": "<p><strong>信息增益 —— \"按这个属性分类能减少多少混乱？\"</strong></p>"
      },
      {
        "type": "code",
        "language": "",
        "code": "Gain(D, A) = H(D) — Σ (|Dv| / |D|) × H(Dv)\n             总混乱   —  分完后的混乱（加权平均）"
      },
      {
        "type": "prose",
        "html": "<p>直觉：如果按\"颜色\"分完后，每一堆都是纯的（只有一种水果），那 Gain 最大。如果按\"重量\"分完每堆还是乱糟糟的，Gain 接近 0。</p>"
      },
      {
        "type": "callout",
        "variant": "tip",
        "text": "考试心法：决策树就是不断选 Gain 最大的属性来划分。谁分完最\"纯\"，就先用谁做节点。走到叶子（都纯了）就停。"
      }
    ]
  },
  {
    "id": "s9-s4",
    "title": "9.3 动手试试 —— 完整决策树推演",
    "content": [
      {
        "type": "prose",
        "html": "<p><strong>以 2025 期末贷款审批为例：14 个样本（9 个批准 ✓，5 个拒绝 ✗）</strong></p>"
      },
      {
        "type": "prose",
        "html": "<p><strong>第 1 步：算总熵</strong></p>"
      },
      {
        "type": "code",
        "language": "",
        "code": "H(D) = —(9/14 × log₂(9/14) + 5/14 × log₂(5/14))\n     = —(0.6429 × log₂(0.6429) + 0.3571 × log₂(0.3571))\n     = —(0.6429 × (—0.6374) + 0.3571 × (—1.4854))\n     = —(—0.4098 — 0.5305)\n     = 0.9403"
      },
      {
        "type": "prose",
        "html": "<p><strong>第 2 步：分别按四个属性划分，算 Gain</strong></p>"
      },
      {
        "type": "details",
        "summary": "按\"年龄\"划分：{不到20, 20~30, 超过30}",
      ...
      {
        "type": "prose",
        "html": "<p><strong>第 3 步：多属性 Gain 对比 —— 选根节点</strong></p>"
      },
      {
        "type": "table",
        "headers": [],
      ...
      {
        "type": "prose",
        "html": "<p><strong>第 4 步：决策树可视化</strong></p>"
      },
      {
        "type": "mermaid",
        "id": "m-fbv6gu",
      ...
      },
      {
        "type": "callout",
        "variant": "tip",
        "text": "注意：考试中关键步骤是把根节点选对——所有候选属性的 Gain 算出来、排大小、选最大。子节点递归同理，每一步都选当前 Gain 最大的。"
      }
    ]
  },
  {
    ...
    "content": [
      {
        "type": "prose",
        "html": "<p style=\"margin-top:var(--space-8)\"><strong>朴素贝叶斯 —— \"数出来的概率\"</strong></p>"
      },
      {
        "type": "prose",
        "html": "<p>直觉：你是动物学家，翻了一本动物图鉴。现在看到一只是陆生、胎生的动物，你要猜它是哺乳动物还是爬行动物。朴素贝叶斯的做法超简单：<strong>从数据表里数</strong>——有多少哺乳动物是陆生的？多少是胎生的？然后乘起来比较。</p>"
      }
    ]
  },
  {
    "id": "s9-s6",
    "title": "9.5 动手试试 —— 完整朴素贝叶斯推演",
    "content": [
      {
        "type": "prose",
        "html": "<p><strong>2023 期末真题：动物分类</strong></p>"
      },
      {
        "type": "prose",
        "html": "<p>假设有 10 个动物样本：</p>"
      },
      {
        "type": "table",
        "headers": [],
      ...
      {
        "type": "prose",
        "html": "<p><strong>问题：一个动物栖息地是 Land、繁殖方式是 Yes（胎生），它是哺乳动物 (+) 还是爬行动物 (—)？</strong></p>"
      },
      {
        "type": "prose",
        "html": "<p><strong>步骤 1：数先验概率 P(类别)</strong></p>"
      },
      {
        "type": "code",
        "language": "",
        "code": "10 个样本中：哺乳 (+) 共 5 个，爬行 (—) 共 5 个\nP(+) = 5/10 = 0.5\nP(—) = 5/10 = 0.5"
      },
      {
        "type": "prose",
        "html": "<p><strong>步骤 2：数条件概率 —— 这就是朴素贝叶斯的核心！</strong></p>"
      },
      {
        "type": "code",
        "language": "",
        "code": "在 5 个哺乳动物 (+) 中：\n  栖息地 = Land 的有几个？猫、狗、人 → 3 个 → P(Land | +) = 3/5 = 0.6\n  繁殖方式 = Yes 的有几个？猫、狗、人、鲸、蝙蝠 → 5 个 → P(Yes | +) = 5/5 = 1.0\n\n在 5 个爬行动物 (—) 中：\n  栖息地 = Land 的有几个？蛇、蜥蜴、龟 → 3 个 → P(Land | —) = 3/5 = 0.6\n  繁殖方式 = Yes 的有几个？没有 → 0 个 → P(Yes | —) = 0/5 = 0"
      },
      {
        "type": "callout",
        "variant": "danger",
        "text": "零概率问题：如果某类中 P(特征|类别) = 0，乘完整个概率就归零了！考试中通常不做拉普拉斯平滑，直接写 0。但你要知道实际应用会用 Laplace 平滑（每项 +1）避免零概率。"
      },
      {
        "type": "prose",
        "html": "<p><strong>步骤 3：乘起来比较</strong></p>"
      },
      {
        "type": "code",
        "language": "",
        "code": "P(+ | Land, Yes) ∝ P(Land | +) × P(Yes | +) × P(+)\n                 = 0.6 × 1.0 × 0.5\n                 = 0.30\n\nP(— | Land, Yes) ∝ P(Land | —) × P(Yes | —) × P(—)\n                 = 0.6 × 0 × 0.5\n                 = 0\n\n结论：0.30 > 0 → 预测为 哺乳动物 ✓"
      },
      {
        "type": "callout",
        "variant": "tip",
        "text": "不用归一化：考试中朴素贝叶斯只需比较\"未归一化乘积\"的大小。谁大就归谁。公式：P(类别|特征) ∝ P(特征₁|类别) × P(特征₂|类别) × P(类别)。"
      }
    ]
  },
  {
    ...
    "content": [
      ...
      },
      {
        "type": "callout",
        "variant": "danger",
        "text": "熵计算时忘记负号 — 正确的熵一定是非负的，如果算出负值立刻检查！$\\log_2(1)=0$ 不是 1。计算器不熟的同学考前练 3 次。"
      },
      {
        "type": "callout",
        "variant": "danger",
        "text": "信息增益 ≠ 条件熵。Gain = H(D) − H(D|A)，减号不能忘。很多人算完条件熵就停了，丢一半分。"
      },
      {
        "type": "callout",
        "variant": "danger",
        "text": "朴素贝叶斯忘记乘先验 P(类别)。只乘 P(特征|类别) 忘了 P(类别)，乘积少了因子。"
      },
      {
        "type": "callout",
        "variant": "danger",
        "text": "决策树递归时只在当前子集上算熵 — 不要把整个数据集再算一遍。每个分支只看走到那个分支的样本。"
      },
      {
        "type": "prose",
        "html": "<h4>答题模板</h4>"
