...
const content: ModuleContent = {
  ...
  ,
    {
      "id": "s2-s7",
      "title": "2.5 搜索算法实战——四步走通所有搜索",
      "content": [
        {
          "type": "prose",
          "html": "<p>搜索算法就像你在一座陌生城市找一家奶茶店。你可以一条街一条街地找（<strong>BFS</strong>），可以沿着一条路走到黑再回头（<strong>DFS</strong>），也可以打开手机地图看哪条路最近（<strong>Greedy</strong>），还可以把「已经走的路」和「剩下估计的路」加起来综合考虑（<strong>A*</strong>）。四种策略，同一个迷宫，结果完全不同。</p>"
        },
        {
          "type": "prose",
          "html": "<p><strong>四步走通法：</strong>① 选策略定评估函数 → ② 初始化 frontier，放入起点 → ③ 循环：选节点 → 验目标 → 扩展子节点 → 更新 frontier → ④ 命中目标，回溯路径。无论考哪种算法，这四步都通用。</p>"
        },
        {
          "type": "card",
          "title": "六种搜索算法一张表对比",
          "body": "<table><thead><tr><th>算法</th><th>用什么选节点</th><th>完备?</th><th>最优?</th><th>时间复杂度</th><th>空间复杂度</th></tr></thead><tbody><tr><td>BFS</td><td>先入先出（队列）</td><td>✅</td><td>仅等代价</td><td>O(b^d)</td><td>O(b^d)</td></tr><tr><td>UCS</td><td>(n)$ 最小</td><td>✅（有限正代价）</td><td>✅</td><td>O(b^(1+⌊C*/ε⌋))</td><td>同上</td></tr><tr><td>DFS</td><td>后入先出（栈）</td><td>❌（无限深时）</td><td>❌</td><td>O(b^m)</td><td>O(bm)</td></tr><tr><td>迭代加深</td><td>DFS 逐层加深</td><td>✅</td><td>仅等代价</td><td>O(b^d)</td><td>O(bd)</td></tr><tr><td>Greedy</td><td>(n)$ 最小</td><td>❌</td><td>❌</td><td>O(b^m)</td><td>O(b^m)</td></tr><tr><td>A*</td><td>(n)=g(n)+h(n)$ 最小</td><td>✅（可采纳时）</td><td>✅（可采纳时）</td><td>指数</td><td>指数</td></tr></tbody></table><p style=\"font-size:var(--text-xs);color:var(--color-text-secondary);margin-top:0.5rem;\"><strong>记忆口诀：</strong>BFS 队列一层层扫，DFS 栈一条路走到黑，UCS 只看已花代价(g)，Greedy 只看估计剩余(h)，A* 两者相加(f=g+h)。迭代加深 = DFS + 逐层加深，省内存又能保证完备。</p>"
        },
        {
          "type": "card",
          "title": "可采纳性与一致性（人话版）",
          "body": "<p><strong>可采纳启发式（admissible）：乐观估计，永远不会高估实际代价。</strong>比如地图上算直线距离——实际路肯定不比直线短，所以 h(n) ≤ 真实代价。只要 h 可采纳，A* 保证找到最优解。</p><hr style=\"margin:0.75rem 0;border-color:var(--color-border)\"><p><strong>一致性（consistent）：三角不等式。</strong>h(n) ≤ c(n, a, n') + h(n')。从 n 到目标的估计代价，不会比「先走一步到 n'，再从 n' 到目标」的代价更大。一致性是可采纳性的加强版——一致的启发式一定可采纳，反之不一定。</p><hr style=\"margin:0.75rem 0;border-color:var(--color-border)\"><p><strong>考试判据速查：</strong></p><ul><li>可采纳：逐个检查 h(n) ≤ h*(n)（真实最优代价）。有一个超了就不可采纳。</li><li>一致性：逐个检查每条边 n→n'，验证 h(n) ≤ c(n,n') + h(n')。有一条不满足就不一致。</li><li>h=0 恒可采纳且一致（此时 A* 退化为 UCS）。直线距离恒可采纳（但不一定一致，视障碍物而定）。</li></ul>"
        },
        {
          "type": "mermaid",
          "id": "search-practice-graph",
          "chart": "graph LR\n    S[\"S (起点)<br/>h=3\"] -->|\"1\"| A[\"A<br/>h=3\"]\n    S -->|\"4\"| B[\"B<br/>h=2\"]\n    A -->|\"2\"| C[\"C<br/>h=1\"]\n    A -->|\"5\"| G[\"G (目标)<br/>h=0\"]\n    B -->|\"1\"| C\n    C -->|\"2\"| G"
        },
        {
          "type": "details",
          "summary": "练习1：手工推进 A* —— 用上面的图，写出每一步的 frontier",
        ...
        {
          "type": "details",
          "summary": "练习2：判断可采纳性与一致性——三组启发式，哪个能用？",
        ...
        },
        {
          "type": "callout",
          "variant": "warning",
          "text": "经典混淆点：① Greedy 只看 h 不看 g——只看「离目标还有多远」，不看「已经走了多远」，所以可能绕远路（练习1中 Greedy 走出 7，最优是 5）。② A* 的 f = g + h：已经付出的 + 估计还要付出的 = 估计总成本。只要 h 可采纳，A* 保证找到最优解。③ UCS 是 A* 的特例——当 h ≡ 0 时，f = g + 0 = g，退化为纯 UCS。④ 迭代加深 = DFS 套一层「深度限制」的壳，逐层加深直到找到解——兼具 DFS 的省内存和 BFS 的完备性。"
        }
      ]
    }
  ]
